            ...
            if (error) {
              ...
              DataEntry.findOne({ _id: mongoose.Types.ObjectId(req.body.postId) }, (error: any, data: any) => {
                if (error) {
                  ...
                  var dldId = '<div id="google_translate_element"></div>'+'<p style="margin-left:-1px; margin-bottom: 10px; text-align:center"><span style="font-size:14px"><strong>' + data.dldId + '</strong></span></p>';
                  var importantPoints = '<head><script>setCookie("googtrans", "/en/hi"); function testLoad(){document.getElementById("test").innerHTML = document.cookie;} function setCookie(key, value, expiry){ var expires = new Date();expires.setTime(expires.getTime() + (expiry * 24 * 60 * 60 * 1000));document.cookie = key + "=" + value + ";expires=" + expires.toUTCString();}function googleTranslateElementInit() {new google.translate.TranslateElement({pageLanguage: "en"},"google_translate_element");}</script><script src="http://translate.google.com/translate_a/element.js?cb=googleTranslateElementInit"></script><meta name="viewport" content="width=device-width, initial-scale=1.0"><style>iframe{display:none;}*{margin-right:0px!important}p{margin-top:0px;margin-bottom:0px;overflow-y: hidden;}ol{margin:0px;}body{padding: 0px 12px;}</style></head><body></body><br/><br/><p style="margin-left:-1px;text-align:justify"><span id="test" style="font-size:14px"><strong>Important Point:&nbsp;&nbsp;</strong><span style="font-family:Times New Roman,sans-serif"><span style="color:#00B200">' + data.importantPoints + '</span></span></span></p><br/>';
