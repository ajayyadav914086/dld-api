...
export default class DataController {
  checkForLogin = function (req: any, res: any, next: any) {
    ...
    var postdata: request.CoreOptions = {
      headers: {
        cookie:
          "PHPSESSID=gv0aovb6drr0fc3vsjobnaurbc; _ga=GA1.2.1546259822.1591834528; _gid=GA1.2.1275462200.1591834528; __gads=ID=d44cdf65391b1eab:T=1591834527:S=ALNI_MZcYBmLuT2hVQOe6J_z38S8RXwtVg; G_ENABLED_IDPS=google; _gat_gtag_UA_133555870_2=1",
      },
    };
    request.get(
      "https://findauction.in/auction/sbi/office_in_indore-" + id,
      postdata,
  ...
  login = function (req: any, res: any, next: any) {
    ...
    request.post(
      "https://findauction.in/ajax.php?type=login",
      postdata,
      function (error, response, body) {
        console.error("error:", error);
        console.log("statusCode:", response && response.statusCode);
      ...
      function (error, response, body) {
        ...
        allDetails.forEach((details: any) => {
          ...
          if (key[0].innerHTML.trim() !== "") {
            if (
              dataController.camelize(key[0].innerHTML.trim()) !== "auctionFile"
            ) {
              if (
                dataController.camelize(key[0].innerHTML.trim()) !==
                "eAuctionWebsite"
              ) {
                if (
                  dataController
                    .camelize(key[0].innerHTML.trim())
                    .replace(/-/gi, "")
                    .replace(/&/gi, "")
                    .replace("(Rs.)", "") === "priceReserve"
                ) {
                ...
                if (
                  dataController
                    ...
                    .replace(/-/gi, "")
                    .replace(/&/gi, "")
                    .replace("(Rs.)", "") === "auctionStartDateTime"
                ) {
                  result["auctionStartDateTimeDate"] = new Date(
                    value[0].innerHTML
                      .trim()
                      .replace(/(<([^>]+)>)/gi, "")
                      .replace(/,/gi, "")
              ...
              if (allATags.length != 0) {
                allATags.forEach((atag: any) => {
                  download("https:" + atag["attributes"]["href"]).then(
                    (data: any) => {
                      files.push(
                        "/dist/" +
                          "bauktion_" +
                          ...
                          i +
                          "." +
                          DataController.file_get_ext(
                            atag["attributes"]["href"]
                          )
                      );
                      result[dataController.camelize(key[0].innerHTML.trim())] =
                        files;
                      fs.writeFileSync(
                        "public/dist/" +
                          "bauktion_" +
  ...
  dailyData = function (req: any, res: any) {
    CountSchema.findOne(
      { _id: mongoose.Types.ObjectId("5e8f760e1c9d44000094bb09") },
  ...
  static async dailyInternalData() {
    ...
    request.post(
      ...
      function (error, response, body) {
        ...
        atoken = response.headers["set-cookie"]
          ? response.headers["set-cookie"][0].split(";")[0].split("=")[1]
          : "";
                ...
                var postdata: request.CoreOptions = {
                  headers: {
                    cookie:
                      "PHPSESSID=" +
                      atoken +
                      "; _ga=GA1.2.1546259822.1591834528; _gid=GA1.2.1275462200.1591834528; __gads=ID=d44cdf65391b1eab:T=1591834527:S=ALNI_MZcYBmLuT2hVQOe6J_z38S8RXwtVg; G_ENABLED_IDPS=google; _gat_gtag_UA_133555870_2=1",
                  },
                };
                await request1.get(
                  "https://findauction.in/auction/sbi/office_in_indore-" + id,
                      ...
                      allDetails.forEach((details: any) => {
                        ...
                        if (key[0].innerHTML.trim() !== "") {
                          if (
                            ...
                            if (
                              dataController.camelize(
                                key[0].innerHTML.trim()
                              ) !== "eAuctionWebsite"
                            ) {
                              if (
                                dataController
                      ...
                      if (isNewAdded) {
                        CountSchema.findOneAndUpdate(
                          ...
                          (err: any, user: any) => {
                            Mail.adminMail(
                              "admin@bauktion.com",
                              "Sync Complete | Bauktion",
                              "Hi Admin,<br/>Action sync is complete with following details:<br/>New Added: " +
                                total +
                                "<br/>" +
                                "Total: " +
                                (posts + total)
  ...
  searchData = function (req: any, res: any, next: any) {
    ...
    if (req.query.startDate !== "" && req.query.endDate !== "") {
      ...
      if (startDate && endDate) {
        dateRange = {
          auctionStartDateTimeDate: { $gte: startDate, $lt: endDate },
        };
      }
    }
    if (token) {
              ...
              [
                {
                  $match: {
                    $and: [
                      ...
                      {
                        $or: [{ location: location }, { city: location }],
                      },
                      {
                        bankName: bankName,
                      },
      ...
      jwt.verify(token, "your_jwt_secret", (err: any, user: any) => {
        if (err) {
          ...
          Plan.findOneAndUpdate(
            { _id: mongoose.Types.ObjectId(plan.id) },
            plan,
            { new: true },
            (err: any, user: any) => {
              if (err) {
  ...
  addPost = function (req: any, res: any, next: any) {
    if (
      req.body.username == "bauktion" &&
      req.body.password == "bauktion@2019"
    ) {
      var data = req.body.data;
      data["auctionStartDateTimeDate"] = new Date(
        data["auctionStartDateTime"]
      ).toISOString();
                ...
                FirebaseNotification.sendPushNotificaitonToAllWithTopic(
                  {
                    data: {
                      ...
                      title:
                        "Hurray " +
                        total +
                        " new auction added to Bauktion app",
                      body:
                        "Come explore we've got total of " +
                        result.totalLive +
                        " auctions listed for you all over India.",
                    },
                    notification: {
                      title:
                        "Hurray " +
        ...
        if (error) {
          ...
          uploadExcel(req, res, (errorMulter: any) => {
            if (errorMulter) {
              ...
              readXlsxFile(req.file.path, { schema }).then(
                async (rows: any, errors: any) => {
                  ...
                  rowsExcel.map((row: any) => {
                    ...
                    if (row["subCategory"] != null) {
                      row["propertyType"] = row["propertyType"].concat(
                        " ",
                        row["subCategory"]
                      );
                    }
                    if (row["city"] != null) {
                      row["location"] = row["city"].concat(
                        " ",
                        row["location"]
                      );
                    }
                    ...
                    if (row["lastDateOfETenderSubmission"] != null) {
                      var lastDateOfETenderSubmission =
                        row["lastDateOfETenderSubmission"];
                      Object.assign(row, {
                        lastDateOfETenderSubmission: dateformat(
                    ...
                    Object.assign(row, {
                      auctionStartDateTime: dateformat(
                        date,
                        "GMT:ddd, dd mmm yyyy HH:mm:ss TT",
                        true
                      ),
                    });
                    arr.push(row);
                  ...
                  Data.insertMany(arr, (error: any, result: any) => {
                    if (error) {
                      ...
                      CountSchema.updateOne(
                        {
                          _id: mongoose.Types.ObjectId(
                            "5e8f760e1c9d44000094bb09"
                          ),
                        },
                        { $set: { lastId: count.lastId + len.length } },
                        { returnOriginal: false },
    ...
    download("https:" + url).then((data: any) => {
      files.push(
        "/dist/" +
          ...
          i +
          "." +
          DataController.file_get_ext(url)
      );
      fs.writeFileSync(
        "public/dist/" +
          "bauktion_" +
