...
export default class UserController {
    ...
    adminLogin = function (req: any, res: any, next: any) {
        if (
            !(req.body.username == "bauktion" && req.body.password == "bauktion@2019")
        ) {
            return res.send({
                message: "unauthorized access",
                responseCode: 700,
    ...
    updateCourtType = async function (req: any, res: any) {
        var token = req.headers.token;
    ...
    sendNotification = function (req: any, res: any, next: any) {
        ...
        var title = req.body.data.title;
        var message = req.body.data.message;
        FirebaseNotification.sendPushNotificaiton(
            user.id,
            {
                data: {
                    type: "1",
            ...
            User.find(function (error: any, result: any) {
                if (error) {
                    ...
                    result.forEach((user: any) => {
                        Mail.adminMail(user.email, title, message)
                            .then((response: any) => {
        ...
        if (token) {
            jwt.verify(token, "your_jwt_secret", (err: any, user: any) => {
                if (err) {
                    ...
                    User.findOneAndUpdate(
                        { _id: mongoose.Types.ObjectId(req.body.userId) },
                        { $set: { planType: req.body.planType } },
                        { new: true, returnOriginal: false },
                        (error: any, updatedUser: any) => {
                            if (err) {
                                return res.send({
                                ...
                                Plans.findOneAndUpdate(
                                    { _id: mongoose.Types.ObjectId(updatedUser.planId) },
                                    { $set: { type: req.body.planType } },
                                    { new: true, returnOriginal: false },
                                    (error: any, plan: any) => {
                                        if (err) {
                                            return res.send({
